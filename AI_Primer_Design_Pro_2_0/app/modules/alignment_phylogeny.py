...
Vollständig stabilisierte Version mit funktionierender Baumvisualisierung.
"""
...
def run_alignment_phylogeny():
    ...
    if mode == "Multiple Sequence Alignment":
        st.subheader("🧬 Multiple Alignment (Demo)")
    ...
    # ------------------------------------------------------------
    # Phylogenetischer Baum (stabile Darstellung)
    # ------------------------------------------------------------
    st.markdown("---")
    ...
    if st.button("🌳 Baum erzeugen"):
        try:
            # --- Sicherstellen, dass Sequenzen gleich lang sind (Padding) ---
            ...
            ax.set_title("Phylogenetischer Baum (Neighbor Joining)", fontsize=12)
            st.pyplot(fig, use_container_width=True)

            # --- ASCII-Vorschau ---
            with st.expander("📜 Baumstruktur (ASCII-Vorschau)"):
                buf = StringIO()
                Phylo.draw_ascii(nj_tree, file=buf)
